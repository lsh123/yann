...
public:
  ...
  void calculate(const RefConstSparseVectorBatch & input, RefVectorBatch output) const;
  void calculate(const RefConstSparseVectorBatch & input, Context * ctx) const;
  ...
  std::unique_ptr<TrainingContext> create_training_context(
...
protected:
  ...
  template<typename InputType>
  Value train_internal(const InputType & input, const RefConstVectorBatch & output, TrainingContext * ctx) const;
